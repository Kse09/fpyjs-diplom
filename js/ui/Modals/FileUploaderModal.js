...
class FileUploaderModal extends BaseModal {
  ...
  registerEvents() {
    ...
    this.content.addEventListener('click', e => {
      const btnUpload = e.target.closest('.image-preview-container button');
      if (btnUpload) {
  ...
  sendImage(imageContainer) {
    ...
    const inputBlock = pathInput.closest('.input');
    ...
    Yandex.uploadFile(pathInput.value.trim(), imageContainer.querySelector('img').src, (err, response) => {
      if (err) {
